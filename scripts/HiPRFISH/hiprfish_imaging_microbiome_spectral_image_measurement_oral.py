...
def save_segmentation(segmentation, sample):
    ...
    plt.axis('off')
    # segfilename = sample + '_adjacency_seg.pdf'
...
def generate_2d_segmentation(image_name, sample):
    # excitations = ['488', '514', '561', '633']
    # image_name = ['{}_{}.czi'.format(sample, x) for x in excitations]
    image_stack = []
    for filename in image_name:
        np_filename = re.sub('.czi','.npy',filename)
        if os.path.exists(np_filename):
            image_stack.append(np.load(np_filename))
        else:
            im = bioformats.load_image(filename)
            image_stack.append(im)
    # image_stack = [im[4000:4500,4000:4500,:] for im in image_stack]
    # image_stack = [bioformats.load_image(filename) for filename in image_name]
    min_r = np.min([im.shape[0] for im in image_stack])
    min_c = np.min([im.shape[1] for im in image_stack])
    image_stack = [im[:min_r, :min_c,:] for im in image_stack]
    ...
    image_lp_rel_norm = image_lp/(image_lp_max[:,:,:,None] + 1e-8)
...
def measure_biofilm_images_2d(image_name):
    # sample = re.sub('_488.czi', '', image_name[0])
    sample = re.sub('_\d{3}.\w+', '', image_name[0])
    image_registered_sum, image_registered, image_final_bkg_filtered, segmentation, adjacency_seg = generate_2d_segmentation(image_name, sample)
    np.save('{}_registered.npy'.format(sample), image_registered)
...
def measure_biofilm_images_2d_from_zstack(image_name, image_stack, z):
    sample = re.sub('_488.npy', '', image_name[0])
    # sample = re.sub('_488.czi', '', image_name[0])
...
def main():
    ...
    if args.dimension == 2:
        if args.zslices == -1:
            measure_biofilm_images_2d(args.image_name)
        else:
            print(args.zslices)

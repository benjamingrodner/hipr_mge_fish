...
def remove_spurious_objects_and_debris(image_seg, image_identification, image_identification_barcode, adjacency_seg, taxon_lookup, cell_info, sample):
    image_identification_filtered = image_identification.copy()
    image_identification_barcode_filtered = image_identification_barcode.copy()
    for i in range(cell_info.shape[0]):
        ...
        if (cell_area > 10000):
          ...
          image_identification_barcode_filtered[image_seg == cell_label] = 0
    ...
    missed_count = 0
    missed_count_filter = 0
    print('\n','cell_info.shape[0]',cell_info.shape[0],'\n')
    print('\n','len(list(rag.edges))',len(list(rag.edges)),'\n')
    missed_edges = []
    for i in range(cell_info.shape[0]):
        try:
    ...
    print('\nmissed count filter]',missed_count_filter,'\n')
    ...
    return(cell_info_filtered, image_identification_filtered, image_identification_barcode_filtered)
    ...
    for q in range(0, len(cell_barcodes)):
      ...
      for r in cell_population.index:
          ...
          if (cell_barcodes.cell_barcode.values[q] in taxon_lookup.cell_barcode.values):
              ...
              pass
    return(image_identification, image_identification_barcode)
...
def main():
    ...
    image_identification, image_identification_barcode = generate_identification_image(image_seg, cell_info, sample, taxon_lookup)
    cell_info_filtered, image_identification_filtered, image_identification_barcode_filtered = remove_spurious_objects_and_debris(image_seg, image_identification, image_identification_barcode, adjacency_seg, taxon_lookup, cell_info, sample)
    ...
    np.save('{}_identification_barcode.npy'.format(sample), image_identification_barcode_filtered)
    save_identification(image_identification_filtered, sample)

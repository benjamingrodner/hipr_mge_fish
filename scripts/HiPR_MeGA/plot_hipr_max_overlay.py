...
def main():
    ...
    parser.add_argument('-ohm', '--overlay_hipr_max_fns', dest = 'overlay_hipr_max_fns', nargs = '+', default = [])
    # parser.add_argument('-hmr', '--hipr_max_resize_fn', dest = 'hipr_max_resize_fn', type = str, default = '', help = 'Input normalized single cell spectra filename')
    ...
    # Plot spots on Airy
    d = config['overlay_hipr_max']
    hipr_max = np.load(args.hipr_max_resize_fn)
    for props_fn, out_fn in zip(
            args.mega_spot_props_shift_fns, args.overlay_hipr_max_fns):
        # Load hiprfish image

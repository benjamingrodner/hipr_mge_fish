...
def main():
    ...
    parser.add_argument('-oh', '--overlay_hipr_fns', dest = 'overlay_hipr_fns', nargs = '+', default = [])
    ...
    # Plot spots on seg
    d = config['overlay_hipr_col']
    hipr_col = np.load(args.hipr_seg_col_resize_fn)
    for props_fn, out_fn in zip(
            args.mega_spot_props_shift_fns, args.overlay_hipr_fns):
        # Load hiprfish image
        # Load spot seg properties
        spot_props_shift = pd.read_csv(props_fn)
        # Plot col
        mge_res = config_mega['resolution']
        fig, ax, cbar = ip.plot_image(hipr_col, scalebar_resolution=mge_res, im_inches=d['im_inches'])
        # Plot spots

...
# %% codecell
thr_pk_area = 400  # Maximum object size (in pixels)
...
# Plot number of pixels that are potential spots normalized by number of 16s pixels.
...
debris_bar_fmt = config['output_dir'] + '/3b_pixel_norm_barplot_chan_{spot_chan}'
for s_ch in config['spot_seg']['channels']:
    fig, ax = ip.general_plot(dims=dims, lw=lw, ft=ft)
    print(s_ch)
    for i, (sn, c, ls) in enumerate(zip(sample_names, cols, linestyles)):
        ...
        bg_mask_area = ip.load_output_file(config, 'spot_mask_bg_fmt', sn, spot_chan=s_ch)
        ...
        print(pix_mask_area)
        pix_norm = pix_mask_area / pix_16s
        ax.bar(i, pix_norm, ls=ls, fill=True, ec=c, width=w)
    ax.set_xticklabels([])
    ax.set_xlim(xlims)
    # save plot

...
# %% codecell
# Absolute path
project_workdir = '/workdir/bmg224/manuscripts/mgefish/code/fig_4/multicolor_phage/segmentation'
...
# %% codecell
bg_thr = 0.001
thresh_pick = [0.002,0.005,0.0015]
...
lw=1
ylims=(0,0.01)
for s_ch, thr_pk, clr in zip(config['spot_seg']['channels'], thresh_pick, cols):
    fig, ax = ip.general_plot(dims=dims, lw=lw, ft=ft)
    print('channel: ', s_ch)
    for sn in sample_names:
        raw = ip.load_output_file(config, 'raw_fmt', sn)
...
# %% codecell
thr_pk_int = [0.0025, 0.007, 0.0025]
ylims=(0,0.04)
dims=(5,5)
lw=1
s=1

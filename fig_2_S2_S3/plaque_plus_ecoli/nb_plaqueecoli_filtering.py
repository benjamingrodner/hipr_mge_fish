...
# %% codecell
# Absolute path
project_workdir = '/fs/cbsuvlaminck2/workdir/bmg224/manuscripts/mgefish/code/fig_3/plaque_plus_ecoli'
...
# %% codecell
# Make the snr folder if not made yet
with open(config_fn, 'r') as f:
    config = yaml.safe_load(f)

spot_mask_bg_rough_fmt = config['output_dir'] + '/' + config['spot_props_snr_fmt']
for sn in sample_names:
...
ft=6
dpi=1000
threshold_picks = [7,7,7,7,7,7,7,7,7,7,7,7,7]
for ch in config['spot_seg']['channels']:
...
n_cores = 4  # number of allowed cores for the snakemake to use
...
# $ cd /fs/cbsuvlaminck2/workdir/bmg224/manuscripts/mgefish/code/fig_3/plaque_plus_ecoli

...
# %% codecell
# Absolute path
project_workdir = '/workdir/bmg224/manuscripts/mgefish/code/fig_3/fig_3b'
...
for s_ch in config['spot_seg']['channels']:
    print('Spot channel ', s_ch)
    fig1, ax1 = apl.general_plot(dims=dims, ft=ft, lw=lw)
    fig2, ax2 = apl.general_plot(dims=dims, ft=ft, lw=lw)
    for i, (sn, c) in enumerate(zip(sample_names, cols)):
        ...
        norm_area = n_spots / area_cell
        norm_count = n_spots / n_cells
        # print('spots total', spot_props.shape[0])
        # print('spots with cell ids', n_spots)
        # plot values as bar plot
        bars1 = ax1.bar(i, norm_area, color=c)
        # ax1.bar_label(bars1, labels=[str(n_cells) + '\ntotal\ncells'])
        bars2 = ax2.bar(i, norm_count, color=c, width=width, edgecolor=edgecolor)
        # ax2.bar_label(bars2, labels=[str(n_cells) + '\ntotal\ncells'])
        print('Sample name: ', sn)
    ...
    plt.figure(fig2)
    ax2.set_yticks(ticks=[0,0.005,0.01])
    ax2.set_yticklabels([])
    ax2.tick_params(axis='y', length=tick_length)

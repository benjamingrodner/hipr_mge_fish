...
# %% codecell
# Absolute path
project_workdir = '/fs/cbsuvlaminck2/workdir/bmg224/manuscripts/mgefish/code/fig_3/two_volunteer_control'
...
out_dir = config['figure_dir'] + '/segmentation'
if not os.path.exists(out_dir): os.makedirs(out_dir)
out_bn = out_dir + '/' + sn + '_cell'
ip.plt.figure(fig)
# dpi = min(isz_rgb.shape)//im_inches
ip.save_png_pdf(out_bn, bbox_inches=0)
...
ip.plot_seg_outline(ax, im_cell_seg, col=(0,1,0))

out_dir = config['figure_dir'] + '/segmentation'
if not os.path.exists(out_dir): os.makedirs(out_dir)
out_bn = out_dir + '/' + sn + '_cell_seg'
ip.plt.figure(fig)
# dpi = min(isz_rgb.shape)//im_inches
ip.save_png_pdf(out_bn, bbox_inches=0)
print('Wrote:',out_bn + '.png')
...
im_spot_mask, im_spot_pre, im_spot_seg = [sf.np.zeros((2,2))]*3
im_spot_seg[0,0] = 1
...
ip.subplot_square_images(
    [
        ...
        im_spot_mask_old[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]
    ], 
    (1,2), 
    clims=[clims,[]]
)
...
ip.subplot_square_images(
    [
        im_spot[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]], 
        im_spot_mask[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]], 
    (1,2), 
    clims=[clims,[]]
)
...
# %% codecell
fig, ax, cbar = ip.plot_image(im_spot[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]], cmap='inferno', clims=clims[1])
...
fig, ax, cbar = ip.plot_image(im_spot[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]],
                              cmap='gray', im_inches=20, clims=clims_spot)
ax.scatter(ma[:,1],ma[:,0], s=75, color=(0,1,0))
ax = ip.plot_seg_outline(ax, im_spot_seg, col=(1,1,1))
# ax.set_xlim((300,400))
# ax.set_ylim((425,475))
...
clims_spot = (0.0, 0.15)
icz = im_cell[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]
isz = im_spot[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]
isz_norm = norm(isz, clims_spot)
isz_rgb = isz_norm[:,:,None] * np.array([1,0,1,1])[None,None,:]
fig, ax, cbar = ip.plot_image(
    icz,
    cmap='gray',
    im_inches=20,
...
ax.imshow(isz_rgb)
ax.scatter(ma[:,1],ma[:,0], s=200, color=(0,1,0))
# ax = ip.plot_seg_outline(ax, im_cell_seg, col=(1,1,1))

out_dir = config['figure_dir'] + '/segmentation'
if not os.path.exists(out_dir): os.makedirs(out_dir)
out_bn = out_dir + '/' + sn + '_spot_count'
ip.plt.figure(fig)
dpi = min(isz_rgb.shape)//im_inches
ip.save_png_pdf(out_bn, bbox_inches=0)
print('Wrote:',out_bn + '.png')
...
force_run = 'segment_spots'  # Pick a rule to re-run. False if you don't want a force run.

...
# %% codecell
# Absolute path
project_workdir = '/fs/cbsuvlaminck2/workdir/bmg224/manuscripts/mgefish/code/fig_4/multicolor_phage/segmentation
...
# %% codecell
sample_names = [sample_names[8], sample_names[10]]
props_area = []
for sn in sample_names:
    pas = []
    for c_ch in config['cell_seg']['channels']:
        # Get regionprops
        props = apl.load_output_file(config, 'cell_props_fmt', sn, cell_chan=c_ch)
...
# %% codecell
threshold_picks_area = [[0, 50000]]
ylims=(0,5000)
dims=(10,5)
lw=1
fig, ax = apl.general_plot(dims=dims, lw=lw, ft=ft)
for sn, pas in zip(sample_names, props_area):
    for s_ch, pa in zip(config['spot_seg']['channels'], pas):
        ax.plot(np.arange(pa.shape[0]), pa, lw=lw)
        xlims = ax.get_xlim()
for thr_pk_area in threshold_picks_area:
    for tpa in thr_pk_area:
        ax.plot(xlims, [tpa]*2, lw=lw*2)
ax.set_ylim(ylims[0],ylims[1])
...
for sn in sample_names:
    for ch, th_pck in zip(config[seg_type]['channels'], threshold_picks_area):
...
dims=(2,2)
ticks=[]
tab10 = apl.get_cmap_listed('tab20')
cols = tab10
...
# t_lims = (0,20)  # lower and upper limits on the possible SNR thresholds
# n_thr = 200  # number of divisions on the threshold limits
# r = 100  # number of points to include in r squared value calculation
# xv = 0.2  # R squared value used set as the filter threshold
# vals = 'snr'
# seg_type = 'spot_seg'
# threshs = np.linspace(t_lims[0], t_lims[1], n_thr)
# spot_snr_curve_dict = {}
# for sn in sample_names:
#     spot_snr_curve_dict[sn] = {}
#     for ch in config['spot_seg']['channels']:
#         spot_snr_curve_dict[sn][ch] = {}
#         props = apl.load_output_file(config, 'spot_props_snr_fmt', sn, spot_chan=ch,
#                                      cell_chan=c_ch)
#         # Get the fraction of spots after thresholding
#         n = props.shape[0]
#         curve = [props[props[vals] > t].shape[0] / n for t in threshs]
#         # Get r squared values
#         rvals = []
#         for i in range(n_thr-r):
#             curve_alt = curve[i:i+r] + np.arange(r)*np.diff(threshs[:2])**2
#             rvals.append(stats.linregress(threshs[i:i+r], curve_alt).rvalue**2)
#         # Get the index where we start removing spots through thresholding
#         start = int(np.ceil(np.interp(0.9, np.flip(curve), np.flip(np.arange(n_thr)))))
#         # Get the threshold value where the r value reaches 'xv'
#         thr = np.interp(xv, rvals[start:],threshs[start:-r])
#         rvals += [0]*r
#         d_curve = np.diff(curve) / np.diff(threshs[:2])
#         dd_curve = np.diff(d_curve) / np.diff(threshs[:2])
#         spot_snr_curve_dict[sn][ch]['curve'] = curve[:-1]
#         spot_snr_curve_dict[sn][ch]['rvals'] = rvals
#         spot_snr_curve_dict[sn][ch]['thr'] = thr
#
# # %% md
#
...
#     for sn in sample_names:
#         for d in ['curve','rvals']:
#             print('Sample: ', sn, '\nChannel: ', ch)
#             curves = [spot_snr_curve_dict[sn][ch][d]]
#             fig, ax = apl.plot_threshold_curves(threshs[:len(curves[0])], curves, xlims=xlims,
#                                                 dims=dims, lw=lw, ft=ft)
#             # for c in curves:
#             #     slope = apl.get_curve_slope(threshs.tolist(), c)
#             #     ax.plot(threshs, slope/np.max(slope))
#             ylims = ax.get_ylim()
#             thr = spot_snr_curve_dict[sn][ch]['thr']
#             ax.plot([thr]*2, ylims, 'k', lw=lw)
...
for sn in sample_names:
    ...
    for s_ch in config['spot_seg']['channels']:
        # Get regionprops
        props = apl.load_output_file(config, 'spot_props_snr_fmt', sn, spot_chan=s_ch, cell_chan=c_ch)
        pas.append(props['snr'].sort_values())
        # Get curve
        # Plot curve and threshold
    props_snr.append(pas)

# %% codecell
threshold_picks = [3.75]
ylims=(0,20)
dims=(10,5)
lw=1
ft=12
fig, ax = apl.general_plot(dims=dims, lw=lw, ft=ft)
for sn, pas in zip(sample_names, props_snr):
    for s_ch, pa in zip(config['spot_seg']['channels'], pas):
        ax.plot(np.arange(pa.shape[0]), pa, lw=lw)
        xlims = ax.get_xlim()
for tpa in threshold_picks:
...
spot_seg_fmt = config['output_dir'] + '/' + config['spot_seg_fmt']
spot_seg_snr_filt_fmt = config['output_dir'] + '/' + config['spot_seg_snr_filt_fmt']
for sn in sample_names:
    raw = apl.load_output_file(config, 'raw_fmt', sn)
    for ch in config['spot_seg']['channels']:
        raw_ch = raw[:,:,ch]
        seg_fn = spot_seg_fmt.format(sample_name=sn, spot_chan=ch)
...
for sn in sample_names:
    for ch, th_pck in zip(config[seg_type]['channels'], threshold_picks):
        raw = apl.load_output_file(config, 'raw_fmt', sn, ch)[:,:,ch]
        # seg = apl.load_output_file(config, 'spot_seg_fmt', sn, ch)

        fig, ax, cbar = apl.plot_image(raw, cmap='inferno', im_inches=10, clims=clims)
        plt.show()
        plt.close()
        # ax.set_ylim(zc[0],zc[1])
        # ax.set_xlim(zc[2],zc[3])
        # plt.sca(ax)
        # output_basename = config['output_dir'] + '/' + sample_names[0] + '_spot_seg_raw'
        # apl.save_png_pdf(output_basename)

        # plot unfiltered image
        fig, ax, cbar = apl.plot_image(raw, cmap='inferno', im_inches=10, clims=clims)
        props = apl.load_output_file(config, 'spot_props_snr_filt_fmt', sn, spot_chan=ch,
                                             cell_chan=c_ch)
        sc = props.centroid.apply(lambda x: eval(x))
        sc = np.array([[i for i in k] for k in sc])
        ax.plot(sc[:,1], sc[:,0], 'o', color=spot_col, ms=marker_size, markerfacecolor='none')
        plt.show()
        plt.close()
        # ax.set_ylim(zc[0],zc[1])
        # ax.set_xlim(zc[2],zc[3])
        # plt.sca(ax)
        # output_basename = config['output_dir'] + '/' + sample_names[0] + '_spot_seg_unfilt'
        # apl.save_png_pdf(output_basename)

        # PLot filtered  image with spot ids
        fig, ax, cbar = apl.plot_image(raw, cmap='inferno', im_inches=10, clims=clims)
        props = apl.load_output_file(config, 'spot_props_snr_filt_fmt', sn, spot_chan=ch,
                                             cell_chan=c_ch)
        sc = props[props.snr_thresh > 0].centroid.apply(lambda x: eval(x))
        sc = np.array([[i for i in k] for k in sc])
        ax.plot(sc[:,1], sc[:,0], 'o', color=spot_col, ms=marker_size, markerfacecolor='none')
        plt.show()
        plt.close()
        ...
        # ax.set_xlim(zc[2],zc[3])
        # plt.sca(ax)
        # output_basename = config['output_dir'] + '/' + sample_names[0] + '_spot_seg_filt'
        # apl.save_png_pdf(output_basename)
...
# $ cd PATHTOREPO/code/fig_5/fig_5b

...
for sn in sample_names:
    raw = ip.load_output_file(config, 'raw_fmt', sn)
    for s_ch, cl in zip(config['spot_seg']['channels'], clims):
        raw_ch = raw[:,:,s_ch]
        mask_new = ip.load_output_file(config, 'spot_mask_bg_fmt', sn, spot_chan=s_ch)
        raw_plot = raw_ch[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]
        mask_plot = mask_new[zc[0]:zc[0]+zs[0],zc[1]:zc[1]+zs[1]]
        fig, ax, cbar = ip.plot_image(raw_plot, cmap='inferno', clims=cl, im_inches=im_inches)
        ip.plot_seg_outline(ax, mask_plot, col=(0,1,0))
...
for sn in sample_names:
    ...
    for s_ch in config['spot_seg']['channels']:
        ...
        ip.subplot_square_images(im_list, (1,3), clims=[clims,clims,clims], im_inches=im_inches)

...
xv = 0.9999  # R squared value used set as the filter threshold
vals = 'snr'
seg_type = 'spot_seg'
...
for sn in sample_names:
    for ch, thr_pk in zip(config['spot_seg']['channels'], threshold_picks):
        props = apl.load_output_file(config, 'spot_props_snr_fmt', sn, spot_chan=ch,
                                     cell_chan=c_ch)
        thr = spot_snr_curve_dict[sn][ch]['thr']
        props['snr_thresh'] = (props.snr > thr).astype(int)
...
# %% codecell
# PLot raw image
zc = (5000, 7000, 3000, 5000)
clims = (0,0.01)
spot_col=(0,1,1)
marker_size=10

fig, ax, cbar = apl.plot_image(raw_ch, cmap='inferno', im_inches=10, clims=clims)
...
n_cores = 18  # number of allowed cores for the snakemake to use
